    ...
    out float4 oColor : OUTCOLOR0)
{
    ...
    // Get fog factor
        ...
        #if defined(MATCAP)
            float3 nm_z = normalize(reflect(iReflectionVec, normal));
            float3 nm_x = float3(nm_z.z, 0.0, -nm_z.x);
            float3 nm_y = cross(nm_x, nm_z);
            float2 vN = 0.5 + 0.5 * float2(dot(normal, nm_x), dot(normal, nm_y));
            //float3 r = normalize(iReflectionVec);
            //float m = 2.0 * sqrt( pow( r.x, 2.0 ) + pow( r.y, 2.0 ) + pow( r.z + 1.0, 2.0 ) );
            //float2 vN = r.xy / m + 0.5;
            //finalColor = Sample2D( EmissiveMap, vN ).rgb;
            finalColor = Sample2D(EmissiveMap, vN).rgb;
